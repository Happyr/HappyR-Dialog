...
!function ($) {
    ...
    var HappyrDialog = function (element, options) {
        ...
        if(this.options.remote){
            this.$element.find('.happyr-dialog-body').load(this.options.remote);
        }
    }
    ...
    $(document).on('click.happyr-dialog.data-api', '[data-toggle="happyr-dialog"]', function (e) {
        ...
        if($target.length == 0){
            ...
            $target=$("#happyr-dialog-auto-generated");

            //if target still not found
            if($target.length == 0){
                //Create a target
                $target=$('<div id="happyr-dialog-auto-generated" style="display:none" tabindex="-1" role="dialog" aria-hidden="true" />')
                    .appendTo(document.body);
            }
        }
